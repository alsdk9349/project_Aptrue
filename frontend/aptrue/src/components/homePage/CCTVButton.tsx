...
interface CCTVButtonProps {
  activeZone: string;
  cctvZone: string[];
}

export default function CCTVButton({ activeZone, cctvZone }: CCTVButtonProps) {
  const router = useRouter();

  const handleButtonClick = (zone: string) => {
    router.push(`/${zone}`); // URL 변경
  };
  ...
  return (
    <div className={style.container}>
      {cctvZone.map((zone, index) => (
        <Button
          key={index}
          color={activeZone === zone ? 'blue' : 'white'}
          size="webSmall"
          clickedColor="blue"
